...
def get_response(question):
    ...
    #Lager if loop der hvis vi skriver en av de spørsmålene så retunerer den ut svar enten random fra liste eller et svar, dersom den leser at jeg skriver spørsmålen så printer den ut svaret, hvis ikke så printer den ut random fra listen "answers"
    if "hei" in question.lower():
        return random.choice(greetings)
    #hvis den leser at jeg har skrevet spørsmål så retunerer den svaret som er med i sprømsål
